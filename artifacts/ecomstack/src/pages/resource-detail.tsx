    ...
    <div className="resource-page mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:py-10">
      ...
      <div className="mt-8 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_270px]">
        ...
        <aside className="space-y-6 lg:sticky lg:top-24">
          <section className="rounded-2xl border border-[#dce7dd] bg-white p-5 shadow-[0_8px_18px_rgba(25,60,54,.04)]"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1"><Button variant="outline" className={`w-full justify-start border-[#cbdacb] ${saved ? "bg-[#E8F1E6] text-[#193C36]" : "bg-white text-[#193C36] hover:bg-[#E8F1E6]"}`} onClick={handleBookmark} disabled={setBookmark.isPending}>{setBookmark.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Bookmark className={`mr-2 h-4 w-4 ${saved ? "fill-current" : ""}`} />}{saved ? "Saved to your Vault" : "Save for later"}</Button><ShareResourceButton slug={resource.slug} title={resource.title} description={resource.description} className="w-full justify-start border-[#cbdacb] text-[#193C36] hover:bg-[#E8F1E6]" /></div>
