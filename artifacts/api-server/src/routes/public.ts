...
router.get("/me", requireUser, async (req, res): Promise<void> => {
  ...
  res.json(GetMemberResponse.parse({ id: user.clerkId, role: user.role, hasAccess: true, accessSource: "account", onboardingCompleted: Boolean(onboarding), claimStatus: claim?.status ?? "none", lastCheckedAt: claim?.lastCheckedAt?.toISOString() ?? null, resumeSlug: claim?.resumeSlug ?? saved[0]?.bookmark.resumeSlug ?? null, shopifySelfReported: Boolean(preferences?.shopifySelfReportedAt), shopifyReviewStarted: Boolean(preferences?.shopifyReviewStartedAt), marketingOptIn: preferences?.marketingOptIn ?? false, savedResources: saved.map((x) => resourceDto(x.resource, x.resource.coverAssetId ? covers.get(x.resource.coverAssetId) ?? "" : "")), recentResources: recent.map((resource) => resourceDto(resource, resource.coverAssetId ? recentCovers.get(resource.coverAssetId) ?? "" : "")) }));
});
